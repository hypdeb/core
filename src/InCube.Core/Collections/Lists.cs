...
public static class Lists
{
    ...
    /// <param name="result">
    /// The output container for the applied <paramref name="selector" />. If null, result is returned
    /// only.
    /// </param>
    ...
    public static TU[] ParSelect<T, TU>(this IReadOnlyList<T> list, Func<T, TU> selector, TU[]? result = null) =>
        list.ParSelect(selector, 0, list.Count, result);
    ...
    /// <typeparam name="TU">The output type.</typeparam>
    /// <returns>An <see cref="Array" />.</returns>
    public static TU[] ParSelect<T, TU>(this IReadOnlyList<T> list, Func<T, int, TU> selector, TU[]? result = null) =>
        list.ParSelect(selector, 0, list.Count, result);
    ...
    /// <summary>
    /// Assuming the <paramref name="indices" /> are in range of every element in the input <paramref name="enumerable" />,
    /// fetches the <see cref="Items{T}" /> out of each element.
    ...
    /// <summary>
    /// Fetches a slice of the <paramref name="list" /> from <paramref name="startInclusive" /> to
    /// <paramref name="stopExclusive" />.
    /// </summary>
    ...
    /// <typeparam name="T">The type of the <paramref name="list" />.</typeparam>
    /// <returns>An <see cref="IEnumerable{T}" />.</returns>
    public static IEnumerable<T> Slice<T>(this IReadOnlyList<T> list, int startInclusive = default, int? stopExclusive = default) =>
        Enumerables.IntRange(startInclusive, stopExclusive ?? list.Count).Select(i => list[i]);

    /// <summary>
    /// Fetches a slice of the <paramref name="enumerable" /> from <paramref name="startInclusive" /> to the
    /// <paramref name="stopExclusive" />.
    /// </summary>
    ...
    /// <summary>
    /// Fetches a slice of an input <see cref="Array" /> <paramref name="array" /> from <paramref name="startInclusive" /> to
    /// the <paramref name="stopExclusive" />.
    /// </summary>
    ...
    public static ArraySegment<T> Slice<T>(this T[] array, int startInclusive = default, int? stopExclusive = default) =>
        new(array, startInclusive, (stopExclusive ?? array.Length) - startInclusive);

    /// <summary>
    /// Fetches a slice of an input <see cref="ArraySegment{T}" /> <paramref name="arraySegment" /> from
    /// <paramref name="startInclusive" /> to the <paramref name="stopExclusive" />.
    /// </summary>
    ...
    public static bool CollectionEqual<T>(this IReadOnlyCollection<T> left, IReadOnlyCollection<T> right, IEqualityComparer<T> comparer) =>
        left.Count == right.Count && left.SequenceEqual(right, comparer);
    ...
    /// <typeparam name="T">The type of the input collections.</typeparam>
    /// <returns>A boolean indicating whether or not the two collections are equal.</returns>
    public static bool CollectionEqual<T>(this IReadOnlyCollection<T> left, IReadOnlyCollection<T> right) =>
        left.Count == right.Count && left.SequenceEqual(right);

    /// <summary>
    /// An adaptation of <see cref="List{T}.RemoveAll" /> by shifting all elements matching the predicate
    /// <paramref name="match" /> to the back of this list.
    /// </summary>
    /// <typeparam name="T">The type of the input list.</typeparam>
