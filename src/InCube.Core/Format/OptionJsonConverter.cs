...
public class OptionJsonConverter<T> : JsonConverter<Option<T>>
{
    ...
    public override void WriteJson(JsonWriter writer, Option<T> value, JsonSerializer serializer) =>
        value.ForEach(writer.WriteNull, v => serializer.Serialize(writer, v));

    /// <inheritdoc />
    public override Option<T>
        ReadJson(JsonReader reader, Type objectType, Option<T> existingValue, bool hasExistingValue, JsonSerializer serializer) =>
