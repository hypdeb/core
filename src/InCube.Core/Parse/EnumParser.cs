...
public static class EnumParser
{
        ...
        where T : struct
    {
        ...
        if (isAnInteger || !parsedSuccessfully)
        {
            ...
            throw new InvalidOperationException(
                $"Could not parse string '{value}' as enum value of '{enumType.Name}'. Valid options would be {validOptions}.");
        }
    ...
    /// <exception cref="InvalidOperationException">
    /// Throws when <typeparamref name="TEnum" /> is not an enum type and when the
