                                ...
                                IInvariantOption<T, Try<T>>
{
    ...
    public TOut Match<TOut>(Func<Exception, TOut> failure, Func<T, TOut> success) =>
        this.HasValue ? success(this.AsOption.Value) : failure(this.Exception);
    ...
    public Task<TOut> MatchAsync<TOut>(Func<Exception, Task<TOut>> failure, Func<T, Task<TOut>> success) =>
        this.HasValue ? success(this.AsOption.Value) : failure(this.Exception);
    ...
    ITry<TOut> ITry<T>.SelectMany<TOut>(Func<Exception, ITry<TOut>> failure, Func<T, ITry<TOut>> success) =>
        this.SelectMany(x => failure(x).ToTry(), x => success(x).ToTry());
    ...
    /// <inheritdoc cref="ITry{T}.SelectMany{TOut}(System.Func{T,InCube.Core.Functional.ITry{TOut}})" />
    public Task<Try<TOut>> SelectMany<TOut>(Func<T, Task<Try<TOut>>> f) => this.MatchAsync(ex => Task.FromResult(Try.Failure<TOut>(ex)), f);
    ...
    /// <inheritdoc />
    public Try<T> Where(Func<T, bool> predicate) =>
        !this.HasValue || predicate(this.AsOption.Value)
            ? this
