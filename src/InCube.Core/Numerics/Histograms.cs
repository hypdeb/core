        ...
        var binSelector = Functions.Invoke(
            () =>
            {
                if (edges is T[] edgeArray)
                    return lowerEdgeEquals ? v => edgeArray.UpperBound(v, comparer) - 1 : v => edgeArray.LowerBound(v, comparer) - 1;

                var listEdges = edges as List<T> ?? edges.ToList();
                return lowerEdgeEquals ? v => listEdges.UpperBound(v, comparer) - 1 : (Func<T, int>)(v => listEdges.LowerBound(v, comparer) - 1);
            });
